...
# 2) 可选：校验下载完整性（SHA256）
echo "b255bfd2fdea81593cbcbb9154b30511ed4d6aa9a66a93e2e7dfe6bca1335a24  open_source_release_20260222_release_ascend_c_onnx_20260222.tar.gz" | sha256sum -c -
...
## 2.1 维护者：更新 `end2end_simp.onnx` 并重建 `ort_tmp`（release 目录）

当你拿到新的 `end2end_simp.onnx` 后，可用下面命令直接更新：

```bash
REPO_ROOT=/path/to/open_source_release_20260222
NEW_END2END_SIMP_ONNX=/path/to/new/end2end_simp.onnx
RELEASE_ONNX_DIR="$REPO_ROOT/release/ascend_c_onnx"

cp -f "$NEW_END2END_SIMP_ONNX" "$RELEASE_ONNX_DIR/end2end_simp.onnx"

source /usr/local/miniconda3/etc/profile.d/conda.sh
conda activate base

python "$REPO_ROOT/standalone_end2end_simp_runner/scripts/patch_end2end_simp_pipeline.py" \
  --pipeline nms_to_npu_nms_ort \
  --in-onnx "$RELEASE_ONNX_DIR/end2end_simp.onnx" \
  --out-onnx "$RELEASE_ONNX_DIR/end2end_simp.only_dets_labels.ort_tmp.onnx"
```

可选校验（确认 `ort_tmp` 已替换为 `NPUNonMaxSuppressionOrt`）：

```bash
python - <<'PY'
import onnx
p = "/path/to/open_source_release_20260222/release/ascend_c_onnx/end2end_simp.only_dets_labels.ort_tmp.onnx"
m = onnx.load(p)
print("has_NPUNonMaxSuppressionOrt:", any(n.op_type == "NPUNonMaxSuppressionOrt" for n in m.graph.node))
print("has_NonMaxSuppression:", any(n.op_type == "NonMaxSuppression" for n in m.graph.node))
print("outputs:", [o.name for o in m.graph.output])
PY
```

如果你要让 workflow 立即读取新文件（不重新解压 tar 包），把 `release/ascend_c_onnx` 同步到 runner 目录：

```bash
mkdir -p "$REPO_ROOT/standalone_end2end_simp_runner/ascend_c_onnx"
cp -f "$REPO_ROOT/release/ascend_c_onnx/"*.onnx \
  "$REPO_ROOT/standalone_end2end_simp_runner/ascend_c_onnx/"
```
